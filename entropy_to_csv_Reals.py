...
CSV_OUTPUT_PATH_1 = "csv/result/reals/dist/" + PATH
CSV_OUTPUT_PATH_2 = "csv/result/reals/entropy/" + PATH
CSV_OUTPUT_PATH_3 = "csv/result/reals/count/" + PATH
...
with open(CSV_INPUT_PATH, 'r') as csv_file:
    ...
    for row in csv_reader:
        data_dict = {i + 1: float(value) for i, value in enumerate(row)}
...
with open(CSV_OUTPUT_PATH_1, 'w', newline='') as csvfile:
    for n in tqdm(range(0, len(data_list), 2), desc="Processing", ncols=100):
        ...
        def sum_probability_rule(model):
            return sum(model.m[i, j] for i in range(1, SIZE) for j in range(1, SIZE)) == 1
        model.sum_probability_constraint = Constraint(rule=sum_probability_rule)

        # 目的関数の定義
        def entropy_rule(model):
            entropy = -sum(model.m[i, j] * log(model.m[i, j] + 1e-2) / log(2) for i in range(1, SIZE) for j in range(1, SIZE))
            return entropy
        ...
        for i in range(1, SIZE):
            for j in range(1, SIZE):
                dist.append(round(value(model.m[i, j]), STEP))
